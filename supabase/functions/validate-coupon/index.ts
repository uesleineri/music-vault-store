...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { code, multitrack_id } = await req.json();
    if (!code || !multitrack_id) {
      return new Response(JSON.stringify({ valid: false, error: "code e multitrack_id são obrigatórios" }), {
        status: 400,
    ...
    const result = await validateCoupon(supabase, code, multitrack.price);
